            ...
            with grpc.insecure_channel(self.server_address) as channel:
                ...
                for response in stub.Play(requests, metadata = metadata):
                    ...
                    if response.event == game_pb2.PlayGameResponse.PlayGameResponseEvent.InvalidAction:
                        print('[WARNING]: Server indicated invalid action event. This might have happened if your opponent has been disconnected or game logic has been corrupted.')

                    if response.event == game_pb2.PlayGameResponse.PlayGameResponseEvent.OpponentInvalidAction:
                        print('[WARNING]: Server indicated invalid action event from your opponent. This might have happened if your opponent has been disconnected or game logic has been corrupted.')

                    if response.event == game_pb2.PlayGameResponse.PlayGameResponseEvent.OpponentDisconnected:
                        print('[INFO]: Server lost connection with your opponent. The game is set to be finished with you as a winner.')
